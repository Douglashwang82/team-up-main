...
def list_events():
    ...
    with SessionLocal() as s:
        ...
        rows = s.execute(q.order_by(Event.starts_at.asc()).limit(100)).scalars().all()
...
def join_event(event_id):
    ...
    with SessionLocal() as s:
        try:
            s.execute(EventParticipants.insert().values(event_id=event_id, user_id=user_id))
            s.commit()
...
def leave_event(event_id):
    ...
    with SessionLocal() as s:
        s.execute(EventParticipants.delete().where(
            (EventParticipants.c.event_id==event_id) & (EventParticipants.c.user_id==user_id)
        ))
        s.commit()
